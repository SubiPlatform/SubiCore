...
The RPM spec file provided here is for Subi-Core 1.0.0 and builds on CentOS
...
reference the release tarball available from https://subi.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Subi git repository](https://github.com/subi/subi) but are not part of
...
Currently only `Source10` is used, for the example `subi.conf` file.
...
in the subi git repository. At present only an SVG version of the Subi
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the subi
...
needed when 0.12.1 is released, a proper fix is already in the Subi git
master branch.
...
The `build-usubi.md` file recommends building against BerkeleyDB 4.8.30. Even if
that is the version your Linux distribution ships with, it probably is a good
idea to build Subi Core against a static version of that library compiled
according to the instructions in the `build-usubi.md` file so that any changes
...
The `subi.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `subi-server` package. At this point, most RPM based
...
The `subi-server` package should have SELinux support. How to properly do
...
Subi-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/subi/). He has been
packaging Subi for Fedora at least since 2012.
...
4. I renamed the `subi` package that contains the Qt GUI to `subi-core` as
that appears to be how the general population refers to it, in contrast to
`subi-xt` or `subi-classic`. I wanted to make sure the general population
