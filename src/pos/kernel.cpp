...
static bool CheckAge(const CBlockIndex *pindexTip, const uint256 &hashKernelBlock, int &nDepth)
{
    ...
    int nRequiredDepth = pindexTip->nHeight + 1 >= Params().GetConsensus().nStartShadeFeeDistribution ?
...
bool CheckProofOfStake(const CBlockIndex *pindexPrev, const CTransaction &tx, int64_t nTime, unsigned int nBits, uint256 &hashProofOfStake, uint256 &targetProofOfStake)
{
    ...
    } else
    {
        ...
        int nRequiredDepth = pindexPrev->nHeight + 1 >= Params().GetConsensus().nStartShadeFeeDistribution ?
                    COINBASE_MATURITY_V2 : COINBASE_MATURITY;
    ...
    {
        if(pindexPrev->nHeight + 1 < Params().GetConsensus().nStartShadeFeeDistribution)
            return state.DoS(100, error("%s: LPoS contract cannot be staked now.", __func__), REJECT_INVALID, "nStartShadeFeeDistribution-start-too-soon");
