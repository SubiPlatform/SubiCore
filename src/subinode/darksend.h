...
// A helper object for signing messages from Subinodes
...
public:
     ...
     *     1) we have an active Subinode,
     *     2) we have a valid Subinode private key,
     *     3) we signed the message successfully, and
     *     4) we verified the message successfully
     */
    bool Sign();
    /// Check if we have a valid Subinode address
    bool CheckSignature(const CPubKey& pubKeySubinode);
    ...
    std::string ToString()
    {
        return strprintf("nDenom=%d, nTime=%lld, fReady=%s, fTried=%s, subinode=%s",
...
{
public:
    /// Is the input associated with this public key? (and there is 10000 SUBI - checking if valid subinode)
...
private:
    ...
    // Keep track of the used Subinodes
    std::vector<CTxIn> vecSubinodesUsed;
    ...
    std::vector<CDarkSendEntry> vecEntries; // Subinode/clients entries
    ...
    /// As a client, submit part of a future mixing transaction to a Subinode to start the process
    ...
    /// Get Subinode updates about the progress of mixing
...
public:
    CSubinode* pSubmittedToSubinode;
