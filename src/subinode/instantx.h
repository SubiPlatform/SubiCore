...
/*
    At 15 signatures, 1/2 of the subinode network can be owned by
...
private:
    ...
    //track subinodes who voted with no txreq (for DOS protection)
    std::map<COutPoint, int64_t> mapSubinodeOrphanVotes; // mn outpoint - time
    ...
    int64_t GetAverageSubinodeOrphanVoteTime();
...
private:
    uint256 txHash;
    COutPoint outpoint;
    COutPoint outpointSubinode;
    std::vector<unsigned char> vchSubinodeSignature;
    // local memory only
...
public:
    CTxLockVote() :
        ...
        outpoint(),
        outpointSubinode(),
        vchSubinodeSignature(),
        nConfirmedHeight(-1),
        nTimeCreated(GetTime())
        {}

    CTxLockVote(const uint256& txHashIn, const COutPoint& outpointIn, const COutPoint& outpointSubinodeIn) :
        txHash(txHashIn),
        outpoint(outpointIn),
        outpointSubinode(outpointSubinodeIn),
        vchSubinodeSignature(),
        nConfirmedHeight(-1),
        nTimeCreated(GetTime())
        {}
    ...
    inline void SerializationOp(Stream& s, Operation ser_action) {
        READWRITE(txHash);
        READWRITE(outpoint);
        READWRITE(outpointSubinode);
        READWRITE(vchSubinodeSignature);
    }
    ...
    COutPoint GetSubinodeOutpoint() const { return outpointSubinode; }
...
private:
    ...
    std::map<COutPoint, CTxLockVote> mapSubinodeVotes; // subinode outpoint - vote
...
public:
    ...
    bool HasSubinodeVoted(const COutPoint& outpointSubinodeIn) const;
    int CountVotes() const { return mapSubinodeVotes.size(); }
...
public:
    ...
    bool HasSubinodeVoted(const COutPoint& outpointIn, const COutPoint& outpointSubinodeIn);
