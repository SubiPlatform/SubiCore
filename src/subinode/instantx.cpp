...
// step 2) Top COutPointLock::SIGNATURES_TOTAL subinodes per each spent outpoint push "txvote" message
...
//    if(!sporkManager.IsSporkActive(SPORK_2_INSTANTSEND_ENABLED)) return;
    ...
    // Ignore any InstantSend messages until subinode list is synced
...
bool CInstantSend::ProcessTxLockRequest(const CTxLockRequest& txLockRequest)
{
    ...
    // Subinodes will sometimes propagate votes before the transaction is known to the client.
...
void CInstantSend::Vote(CTxLockCandidate& txLockCandidate)
{
    ...
    while(itOutpointLock != txLockCandidate.mapOutPointLocks.end()) {
        ...
        int n = mnodeman.GetSubinodeRank(activeSubinode.vin, nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION);

        if(n == -1) {
            //LogPrint("instantsend", "CInstantSend::Vote -- Unknown Subinode %s\n", activeSubinode.vin.prevout.ToStringShort());
            ++itOutpointLock;
            continue;
        }
        ...
        if(n > nSignaturesTotal) {
            //LogPrint("instantsend", "CInstantSend::Vote -- Subinode not in the top %d (%d)\n", nSignaturesTotal, n);
            ++itOutpointLock;
            continue;
        }
        ...
        if(itVoted != mapVotedOutpoints.end()) {
            BOOST_FOREACH(const uint256& hash, itVoted->second) {
                ...
                if(it2->second.HasSubinodeVoted(itOutpointLock->first, activeSubinode.vin.prevout)) {
        ...
        CTxLockVote vote(txHash, itOutpointLock->first, activeSubinode.vin.prevout);
...
bool CInstantSend::ProcessTxLockVote(CNode* pfrom, CTxLockVote& vote)
{
    ...
    if(it == mapTxLockCandidates.end()) {
        if(!mapTxLockVotesOrphan.count(vote.GetHash())) {
            ...
            //LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- Orphan vote: txid=%s  subinode=%s new\n",
                //    txHash.ToString(), vote.GetSubinodeOutpoint().ToStringShort());
            ...
            //LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- Orphan vote: txid=%s  subinode=%s seen\n",
                 //   txHash.ToString(), vote.GetSubinodeOutpoint().ToStringShort());
        }
        ...
        int nSubinodeOrphanExpireTime = GetTime() + 60*10; // keep time data for 10 minutes
        if(!mapSubinodeOrphanVotes.count(vote.GetSubinodeOutpoint())) {
            mapSubinodeOrphanVotes[vote.GetSubinodeOutpoint()] = nSubinodeOrphanExpireTime;
        } else {
            int64_t nPrevOrphanVote = mapSubinodeOrphanVotes[vote.GetSubinodeOutpoint()];
            if(nPrevOrphanVote > GetTime() && nPrevOrphanVote > GetAverageSubinodeOrphanVoteTime()) {
                //LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- subinode is spamming orphan Transaction Lock Votes: txid=%s  subinode=%s\n",
                    //    txHash.ToString(), vote.GetSubinodeOutpoint().ToStringShort());
    ...
    if(it1 != mapVotedOutpoints.end()) {
        BOOST_FOREACH(const uint256& hash, it1->second) {
            if(hash != txHash) {
                ...
                std::map<uint256, CTxLockCandidate>::iterator it2 = mapTxLockCandidates.find(hash);
                if(it2->second.HasSubinodeVoted(vote.GetOutpoint(), vote.GetSubinodeOutpoint())) {
                    // yes, it did, refuse to accept a vote to include the same outpoint in another tx
                    // from the same subinode.
                    // TODO: apply pose ban score to this subinode?
                    ...
                    // pose ban score too.
                    //LogPrint("CInstantSend::ProcessTxLockVote -- subinode sent conflicting votes! %s\n", vote.GetSubinodeOutpoint().ToStringShort());
                    return false;
                }
            }
        }
        // we have votes by other subinodes only (so far), let's continue and see who will win
...
int64_t CInstantSend::GetAverageSubinodeOrphanVoteTime()
{
    LOCK(cs_instantsend);
    // NOTE: should never actually call this function when mapSubinodeOrphanVotes is empty
    if(mapSubinodeOrphanVotes.empty()) return 0;

    std::map<COutPoint, int64_t>::iterator it = mapSubinodeOrphanVotes.begin();
    int64_t total = 0;

    while(it != mapSubinodeOrphanVotes.end()) {
        total+= it->second;
        ++it;
    }

    return total / mapSubinodeOrphanVotes.size();
}
...
void CInstantSend::CheckAndRemove()
{
    ...
    while(itVote != mapTxLockVotes.end()) {
        if(itVote->second.IsExpired(pCurrentBlockIndex->nHeight)) {
            //LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired vote: txid=%s  subinode=%s\n",
                //    itVote->second.GetTxHash().ToString(), itVote->second.GetSubinodeOutpoint().ToStringShort());
    ...
    while(itOrphanVote != mapTxLockVotesOrphan.end()) {
        if(GetTime() - itOrphanVote->second.GetTimeCreated() > ORPHAN_VOTE_SECONDS) {
            //LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired orphan vote: txid=%s  subinode=%s\n",
               //     itOrphanVote->second.GetTxHash().ToString(), itOrphanVote->second.GetSubinodeOutpoint().ToStringShort());
    ...
    // remove expired subinode orphan votes (DOS protection)
    std::map<COutPoint, int64_t>::iterator itSubinodeOrphan = mapSubinodeOrphanVotes.begin();
    while(itSubinodeOrphan != mapSubinodeOrphanVotes.end()) {
        if(itSubinodeOrphan->second < GetTime()) {
            //LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired orphan subinode vote: subinode=%s\n",
               //     itSubinodeOrphan->first.ToStringShort());
            mapSubinodeOrphanVotes.erase(itSubinodeOrphan++);
        } else {
            ++itSubinodeOrphan;
        }
    }
...
{
    if(!mnodeman.Has(CTxIn(outpointSubinode))) {
        //LogPrint("instantsend", "CTxLockVote::IsValid -- Unknown subinode %s\n", outpointSubinode.ToStringShort());
        mnodeman.AskForMN(pnode, CTxIn(outpointSubinode));
        return false;
    }
    ...
    int n = mnodeman.GetSubinodeRank(CTxIn(outpointSubinode), nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION);

    if(n == -1) {
        //can be caused by past versions trying to vote with an invalid protocol
        //LogPrint("instantsend", "CTxLockVote::IsValid -- Outdated subinode %s\n", outpointSubinode.ToStringShort());
        return false;
    }
    //LogPrint("instantsend", "CTxLockVote::IsValid -- Subinode %s, rank=%d\n", outpointSubinode.ToStringShort(), n);

    int nSignaturesTotal = COutPointLock::SIGNATURES_TOTAL;
    if(n > nSignaturesTotal) {
        //LogPrint("instantsend", "CTxLockVote::IsValid -- Subinode %s is not in the top %d (%d), vote hash=%s\n",
               // outpointSubinode.ToStringShort(), nSignaturesTotal, n, GetHash().ToString());
        return false;
    }
...
bool CTxLockVote::CheckSignature() const
{
    ...
    subinode_info_t infoMn = mnodeman.GetSubinodeInfo(CTxIn(outpointSubinode));

    if(!infoMn.fInfoValid) {
        //LogPrint("CTxLockVote::CheckSignature -- Unknown Subinode: subinode=%s\n", outpointSubinode.ToString());
        return false;
    }

    if(!darkSendSigner.VerifyMessage(infoMn.pubKeySubinode, vchSubinodeSignature, strMessage, strError)) {
...
bool CTxLockVote::Sign()
{
    ...
    if(!darkSendSigner.SignMessage(strMessage, vchSubinodeSignature, activeSubinode.keySubinode)) {
        //LogPrint("CTxLockVote::Sign -- SignMessage() failed\n");
        return false;
    }

    if(!darkSendSigner.VerifyMessage(activeSubinode.pubKeySubinode, vchSubinodeSignature, strMessage, strError)) {
...
{
    if(mapSubinodeVotes.count(vote.GetSubinodeOutpoint()))
        return false;
    mapSubinodeVotes.insert(std::make_pair(vote.GetSubinodeOutpoint(), vote));
    return true;
}
...
std::vector<CTxLockVote> COutPointLock::GetVotes() const
{
    ...
    std::map<COutPoint, CTxLockVote>::const_iterator itVote = mapSubinodeVotes.begin();
    while(itVote != mapSubinodeVotes.end()) {
...
bool COutPointLock::HasSubinodeVoted(const COutPoint& outpointSubinodeIn) const
{
    return mapSubinodeVotes.count(outpointSubinodeIn);
}
...
bool CTxLockCandidate::HasSubinodeVoted(const COutPoint& outpointIn, const COutPoint& outpointSubinodeIn)
{
    std::map<COutPoint, COutPointLock>::iterator it = mapOutPointLocks.find(outpointIn);
    return it !=mapOutPointLocks.end() && it->second.HasSubinodeVoted(outpointSubinodeIn);
}
