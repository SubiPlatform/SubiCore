...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx)
{
    ...
    if (nHeight >= chainparams.GetConsensus().nSubinodePaymentsStartBlock) {
        CAmount subinodePayment = GetSubinodePayment(nHeight, blockReward);
        coinbaseTx.vout[1].nValue -= subinodePayment;
        FillBlockPayments(coinbaseTx, nHeight, subinodePayment, pblock->txoutSubinode, pblock->voutSuperblock);
    }
