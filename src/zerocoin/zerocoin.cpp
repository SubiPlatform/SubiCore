...
static map<uint256, int64_t> shadedCoins;
...
bool CheckDevFundInputs(const CTransaction &tx, CValidationState &state, int nHeight, bool fTestNet) {
    ...
    if(Params().GetConsensus().nPosHeightActivate > nHeight){
        if(nHeight >= Params().GetConsensus().nSubinodePaymentsStartBlock){

            int total_payment_tx = 0;
            CAmount subinodePayment = GetSubinodePayment(nHeight, 0);

            BOOST_FOREACH(const CTxOut &output, tx.vout) {
                if (subinodePayment == output.nValue) {
            ...
            if (total_payment_tx > 1) {
                return state.DoS(100, false, REJECT_INVALID_SUBINODE_PAYMENT,
                                 "CTransaction::CheckTransaction() : invalid subinode payment");
            }
        }
    }
    //PoS reward distribution
        ...
        /* Check for Subinode payment in block */
        ...
        if(nHeight >= Params().GetConsensus().nSubinodePaymentsStartBlock){
            ...
            BOOST_FOREACH(const CTxOut &output, tx.vout) {
                if (output.nValue >= subinodePayment) {
                    total_payment_tx = total_payment_tx + 1;
                }
            }
            // no more than 1 output for payment, possible no winner if list is not populated
                              ...
                              CZerocoinTxInfo *zerocoinTxInfo)
{
    ...
    if(tx.IsZerocoinSpend()) {
        ...
        BOOST_FOREACH(const CTxOut &txout, tx.vout)
        {
            ...
            shadedCoins[hashTx] = tx.GetValueOut();
            i++;
        }
    }
...
void DisconnectTipShade(CBlock & /*block*/, CBlockIndex *pindexDelete) {
 ...
 */
bool ConnectBlockShade(CValidationState &state, const CChainParams &chainparams, CBlockIndex *pindexNew, const CBlock *pblock) {
...
uint64_t CZerocoinState::GetTotalZerocoins(){
    ...
    //total coins spent
    for(auto it = shadedCoins.begin(); it != shadedCoins.end(); it++) {
