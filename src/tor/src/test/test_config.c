...
test_config_port_cfg_line_extract_addrport(void *arg)
{
  ...
  tt_int_op(port_cfg_line_extract_addrport("", &a, &usubiy, &rest), OP_EQ, 0);
  tt_int_op(usubiy, OP_EQ, 0);
  tt_str_op(a, OP_EQ, "");
  tt_str_op(rest, OP_EQ, "");
  tor_free(a);

  tt_int_op(port_cfg_line_extract_addrport("hello", &a, &usubiy, &rest),
            OP_EQ, 0);
  tt_int_op(usubiy, OP_EQ, 0);
  ...
  tt_int_op(port_cfg_line_extract_addrport("usubi:flipperwalt \t gersplut",
                                           &a, &usubiy, &rest), OP_EQ, 0);
  tt_int_op(usubiy, OP_EQ, 1);
  tt_str_op(a, OP_EQ, "flipperwalt");
  tt_str_op(rest, OP_EQ, "gersplut");
  tor_free(a);
  ...
  tt_int_op(port_cfg_line_extract_addrport("usubi:lolol",
                                           &a, &usubiy, &rest), OP_EQ, 0);
  tt_int_op(usubiy, OP_EQ, 1);
  tt_str_op(a, OP_EQ, "lolol");
  tt_str_op(rest, OP_EQ, "");
  ...
  tt_int_op(port_cfg_line_extract_addrport("usubi:\"lolol\" foo ",
                                           &a, &usubiy, &rest), OP_EQ, 0);
  tt_int_op(usubiy, OP_EQ, 1);
  tt_str_op(a, OP_EQ, "lolol");
  ...
  tt_int_op(port_cfg_line_extract_addrport("usubi:\"lol ol\" foo ",
                                           &a, &usubiy, &rest), OP_EQ, 0);
  tt_int_op(usubiy, OP_EQ, 1);
  ...
  tt_int_op(port_cfg_line_extract_addrport("usubi:\"lol\\\" ol foo ",
                                           &a, &usubiy, &rest), OP_EQ, -1);
  tor_free(a);

  tt_int_op(port_cfg_line_extract_addrport("usubi:\"lol\\0\" ol foo ",
                                           &a, &usubiy, &rest), OP_EQ, -1);
  tor_free(a);
...
test_config_parse_port_config__ports__no_ports_given(void *data)
{
  ...
  tt_int_op(port_cfg->is_usubi_addr, OP_EQ, 0);

  // Test with defaultport, with defaultaddress and out, adds a new port cfg
  // for a usubi address
  SMARTLIST_FOREACH(slout,port_cfg_t *,pf,port_cfg_free(pf));
  smartlist_clear(slout);
  ret = parse_port_config(slout, NULL, "DNS", 0, "/foo/bar/usubidomain",
                          42, CL_PORT_IS_USUBISOCKET);
  tt_int_op(ret, OP_EQ, 0);
  tt_int_op(smartlist_len(slout), OP_EQ, 1);
  port_cfg = (port_cfg_t *)smartlist_get(slout, 0);
  tt_int_op(port_cfg->port, OP_EQ, 0);
  tt_int_op(port_cfg->is_usubi_addr, OP_EQ, 1);
  tt_str_op(port_cfg->usubi_addr, OP_EQ, "/foo/bar/usubidomain");
...
test_config_parse_port_config__ports__ports_given(void *data)
{
  ...
  // Test error when encounters an empty usubi domain specification
  config_free_lines(config_port_invalid); config_port_invalid = NULL;
  config_port_invalid = mock_config_line("DNSPort", "usubi:");
  ret = parse_port_config(NULL, config_port_invalid, "DNS", 0, NULL,
                          0, 0);
  tt_int_op(ret, OP_EQ, -1);

  // Test error when encounters a usubi domain specification but the listener
  // doesn't support domain sockets
  config_port_valid = mock_config_line("DNSPort", "usubi:/tmp/foo/bar");
  ret = parse_port_config(NULL, config_port_valid, "DNS",
                          CONN_TYPE_AP_DNS_LISTENER, NULL, 0, 0);
  tt_int_op(ret, OP_EQ, -1);

  // Test valid usubi domain
  SMARTLIST_FOREACH(slout,port_cfg_t *,pf,port_cfg_free(pf));
  smartlist_clear(slout);
  ...
  tt_int_op(port_cfg->port, OP_EQ, 0);
  tt_int_op(port_cfg->is_usubi_addr, OP_EQ, 1);
  tt_str_op(port_cfg->usubi_addr, OP_EQ, "/tmp/foo/bar");
  ...
  config_port_invalid = mock_config_line("SOCKSPort",
                                         "usubi:/tmp/foo/bar NoIPv4Traffic "
                                         "NoIPv6Traffic "
                                         "NoOnionTraffic");
  ...
  SMARTLIST_FOREACH(slout,port_cfg_t *,pf,port_cfg_free(pf));
  smartlist_clear(slout);
  config_port_valid = mock_config_line("SOCKSPort", "usubi:/tmp/foo/bar "
                                       "NoIPv6Traffic "
  ...
  // Test success with quoted usubi: address.
  config_free_lines(config_port_valid); config_port_valid = NULL;
  SMARTLIST_FOREACH(slout,port_cfg_t *,pf,port_cfg_free(pf));
  smartlist_clear(slout);
  config_port_valid = mock_config_line("SOCKSPort", "usubi:\"/tmp/foo/ bar\" "
  ...
  // Test failure with broken quoted usubi: address.
  config_free_lines(config_port_valid); config_port_valid = NULL;
  SMARTLIST_FOREACH(slout,port_cfg_t *,pf,port_cfg_free(pf));
  smartlist_clear(slout);
  config_port_valid = mock_config_line("SOCKSPort", "usubi:\"/tmp/foo/ bar "
  ...
  // Test failure with empty quoted usubi: address.
  config_free_lines(config_port_valid); config_port_valid = NULL;
  SMARTLIST_FOREACH(slout,port_cfg_t *,pf,port_cfg_free(pf));
  smartlist_clear(slout);
  config_port_valid = mock_config_line("SOCKSPort", "usubi:\"\" "
                                       "NoIPv6Traffic "
                                       "NoDNSRequest NoIPv4Traffic");
  ret = parse_port_config(slout, config_port_valid, "SOCKS",
                          CONN_TYPE_AP_LISTENER, NULL, 0,
  ...
  // Test success with a zero usubi domain socket, and doesnt add it to out
  config_free_lines(config_port_valid); config_port_valid = NULL;
  SMARTLIST_FOREACH(slout,port_cfg_t *,pf,port_cfg_free(pf));
  smartlist_clear(slout);
  config_port_valid = mock_config_line("DNSPort", "0");
  ret = parse_port_config(slout, config_port_valid, "DNS", 0,
                          "127.0.0.45", 0, CL_PORT_IS_USUBISOCKET);
  tt_int_op(ret, OP_EQ, 0);
  tt_int_op(smartlist_len(slout), OP_EQ, 0);

  // Test success with a one usubi domain socket, and doesnt add it to out
  config_free_lines(config_port_valid); config_port_valid = NULL;
  SMARTLIST_FOREACH(slout,port_cfg_t *,pf,port_cfg_free(pf));
  smartlist_clear(slout);
  ...
  port_cfg = (port_cfg_t *)smartlist_get(slout, 0);
  tt_int_op(port_cfg->is_usubi_addr, OP_EQ, 1);
  tt_str_op(port_cfg->usubi_addr, OP_EQ, "something");
  ...
  SMARTLIST_FOREACH(slout,port_cfg_t *,pf,port_cfg_free(pf));
  smartlist_clear(slout);
  config_port_valid = mock_config_line("SOCKSPort", "usubi:/tmp/somewhere");
  ret = parse_port_config(slout, config_port_valid, "SOCKS",
...
test_config_parse_port_config__ports__server_options(void *data)
{
  ...
  // Check for failure with empty usubi: address.
  config_free_lines(config_port_invalid); config_port_invalid = NULL;
  SMARTLIST_FOREACH(slout,port_cfg_t *,pf,port_cfg_free(pf));
  smartlist_clear(slout);
  config_port_invalid = mock_config_line("ORPort", "usubi:\"\"");
