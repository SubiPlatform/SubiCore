...
test_rend_service_parse_port_config(void *arg)
{
  ...
  /* quoted usubi port */
  tor_free(err_msg);
  cfg = rend_service_parse_port_config("100 usubi:\"/tmp/foo bar\"",
                                       " ", &err_msg);
  tt_assert(cfg);
  tt_ptr_op(err_msg, OP_EQ, NULL);
  rend_service_port_config_free(cfg);
  ...
  /* quoted usubi port, missing end quote */
  cfg = rend_service_parse_port_config("100 usubi:\"/tmp/foo bar",
                                       " ", &err_msg);
  tt_ptr_op(cfg, OP_EQ, NULL);
  tt_str_op(err_msg, OP_EQ, "Couldn't process address <usubi:\"/tmp/foo bar> "
