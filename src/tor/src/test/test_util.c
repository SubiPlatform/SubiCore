...
test_util_make_environment(void *ptr)
{
  ...
  smartlist_t *env_vars_in_usubioid_env_block_sorted = smartlist_new();
  ...
  /* Now for the Usubioid environment block.  We don't care which order
  ...
  {
    char **v;
    for (v = env->usubioid_environment_block; *v; ++v) {
      smartlist_add(env_vars_in_usubioid_env_block_sorted, *v);
    }
  }

  smartlist_sort_strings(env_vars_sorted);
  smartlist_sort_strings(env_vars_in_usubioid_env_block_sorted);

  tt_want_int_op(smartlist_len(env_vars_sorted), OP_EQ,
                 smartlist_len(env_vars_in_usubioid_env_block_sorted));
  {
    int len = smartlist_len(env_vars_sorted);
    int i;

    if (smartlist_len(env_vars_in_usubioid_env_block_sorted) < len) {
      len = smartlist_len(env_vars_in_usubioid_env_block_sorted);
    }

    for (i = 0; i < len; ++i) {
      tt_want_str_op(smartlist_get(env_vars_sorted, i), OP_EQ,
                     smartlist_get(env_vars_in_usubioid_env_block_sorted, i));
    }
  }

  /* Clean up. */
  smartlist_free(env_vars_in_usubioid_env_block_sorted);
