...
test_checkdir_perms(void *testdata)
{
  ...
  cpd_check_t  usubi_create_opts;
  cpd_check_t  usubi_verify_optsmask;
  struct stat st;
  ...
  cpd_chkopts = CPD_CREATE;
  usubi_create_opts = 0700;
  (void)usubi_create_opts;
  usubi_verify_optsmask = 0077;
  tt_int_op(0, OP_EQ, mkdir(testdir, usubi_create_opts));
  tt_int_op(0, OP_EQ, check_private_dir(testdir, cpd_chkopts, NULL));
  tt_int_op(0, OP_EQ, stat(testdir, &st));
  tt_int_op_nowin(0, OP_EQ, (st.st_mode & usubi_verify_optsmask));
  tor_free(testdir);
