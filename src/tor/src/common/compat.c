...
/**
 * Rename the file <b>from</b> to the file <b>to</b>.  On Usubi, this is
...
/** @{ */
/** Some old versions of Usubi didn't define constants for these values,
...
tor_close_socket_simple(tor_socket_t s)
{
  ...
  * and closesocket() on fds returned by socket().  On Usubi, everything
...
/** Similar behavior to Usubi gethostbyname: resolve <b>name</b>, and set
...
#define HAVE_USUBI_MLOCKALL
#endif

#ifdef HAVE_USUBI_MLOCKALL
...
#endif /* defined(HAVE_USUBI_MLOCKALL) */
...
#else /* !(defined(HAVE_USUBI_MLOCKALL)) */
