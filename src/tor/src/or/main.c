...
#ifdef SIGINT
  { SIGINT, USUBI_ONLY, NULL }, /* do a controlled slow shutdown */
#endif
#ifdef SIGTERM
  { SIGTERM, USUBI_ONLY, NULL }, /* to terminate now */
#endif
#ifdef SIGPIPE
  { SIGPIPE, USUBI_ONLY, NULL }, /* otherwise SIGPIPE kills us */
#endif
#ifdef SIGUSR1
  { SIGUSR1, USUBI_ONLY, NULL }, /* dump stats */
#endif
#ifdef SIGUSR2
  { SIGUSR2, USUBI_ONLY, NULL }, /* go to loglevel debug */
#endif
#ifdef SIGHUP
  { SIGHUP, USUBI_ONLY, NULL }, /* to reload config, retry conns, etc */
#endif
#ifdef SIGXFSZ
  { SIGXFSZ, USUBI_ONLY, NULL }, /* handle file-too-big resource exhaustion */
#endif
#ifdef SIGCHLD
  { SIGCHLD, USUBI_ONLY, NULL }, /* handle dns/cpu workers that exit */
#endif
...
#define STAT_DATADIR2(name, name2)                                      \
  ...
  SMARTLIST_FOREACH_BEGIN(get_configured_ports(), port_cfg_t *, port) {
    ...
    char *dirname = tor_strdup(port->usubi_addr);
    if (get_parent_directory(dirname) == 0) {
      OPEN(dirname);
    }
    tor_free(dirname);
    sandbox_cfg_allow_chmod_filename(&cfg, tor_strdup(port->usubi_addr));
    sandbox_cfg_allow_chown_filename(&cfg, tor_strdup(port->usubi_addr));
