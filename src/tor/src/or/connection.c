 ...
 * on connections.
 *
 * Each connection (ideally) represents a TLS connection, a TCP socket, a usubi
 ...
 * sockets. */
int
conn_listener_type_supports_af_usubi(int type)
{
  /* For now only control ports or SOCKS ports can be Usubi domain sockets
...
connection_free_(connection_t *conn)
{
  ...
  if (!connection_is_listener(conn)) {
    ...
    if (conn->socket_family == AF_UNIX) {
      /* For now only control and SOCKS ports can be Usubi domain sockets
       * and listeners at the same time */
      tor_assert(conn_listener_type_supports_af_usubi(conn->type));
 ...
 * <b>listenaddress</b> provides the path to the Usubi socket.
 *
...
create_usubi_sockaddr(const char *listenaddress, char **readable_address,
                     socklen_t *len_out)
{
  ...
  if (strlcpy(sockaddr->sun_path, listenaddress, sizeof(sockaddr->sun_path))
    ...
    log_warn(LD_CONFIG, "Usubi socket path '%s' is too long to fit.",
                     ...
                     socklen_t *len_out)
{
  ...
  log_fn(LOG_ERR, LD_BUG,
         "Usubi domain sockets not supported, yet we tried to create one.");
  *len_out = 0;
  tor_fragile_assert();
  return NULL;
}
...
#define USUBI_SOCKET_PURPOSE_CONTROL_SOCKET 0
#define USUBI_SOCKET_PURPOSE_SOCKS_SOCKET 1

/** Check if the purpose isn't one of the ones we know what to do with */

static int
is_valid_usubi_socket_purpose(int purpose)
{
  int valid = 0;

  switch (purpose) {
    case USUBI_SOCKET_PURPOSE_CONTROL_SOCKET:
    case USUBI_SOCKET_PURPOSE_SOCKS_SOCKET:
      valid = 1;
      break;
  }
...
/** Return a string description of a usubi socket purpose */
static const char *
usubi_socket_purpose_to_string(int purpose)
{
...
static int
check_location_for_usubi_socket(const or_options_t *options, const char *path,
                               int purpose, const port_cfg_t *port)
{
  int r = -1;
  char *p = NULL;

  tor_assert(is_valid_usubi_socket_purpose(purpose));

  p = tor_strdup(path);
  cpd_check_t flags = CPD_CHECK_MODE_ONLY;
  if (get_parent_directory(p)<0 || p[0] != '/') {
    log_warn(LD_GENERAL, "Bad usubi socket address '%s'.  Tor does not support "
             "relative paths for usubi sockets.", path);
    goto done;
  }
  ...
  if (check_private_dir(p, flags, options->User) < 0) {
    ...
    log_warn(LD_GENERAL, "Before Tor can create a %s in %s, the directory "
             ...
             "account that is running Tor.  (On some Usubi systems, anybody "
             "who can list a socket can connect to it, so Tor is being "
             "careful.)",
             usubi_socket_purpose_to_string(purpose), escpath, escdir,
                        ...
                        const port_cfg_t *port_cfg)
{
   ...
   */
    ...
    tor_assert(conn_listener_type_supports_af_usubi(type));

    if (check_location_for_usubi_socket(options, address,
          (type == CONN_TYPE_CONTROL_LISTENER) ?
           USUBI_SOCKET_PURPOSE_CONTROL_SOCKET :
           USUBI_SOCKET_PURPOSE_SOCKS_SOCKET, port_cfg) < 0) {
        goto err;
    }
 ...
 */
int
connection_connect_usubi(connection_t *conn, const char *socket_path,
                        int *socket_error)
{
  ...
  SMARTLIST_FOREACH_BEGIN(old_conns, connection_t *, conn) {
    ...
    SMARTLIST_FOREACH_BEGIN(launch, const port_cfg_t *, wanted) {
      if (conn->type != wanted->type)
        continue;
      if ((conn->socket_family != AF_UNIX && wanted->is_usubi_addr) ||
          (conn->socket_family == AF_UNIX && ! wanted->is_usubi_addr))
        continue;

      if (wanted->server_cfg.no_listen)
        continue; /* We don't want to open a listener for this one */

      if (wanted->is_usubi_addr) {
        if (conn->socket_family == AF_UNIX &&
            !strcmp(wanted->usubi_addr, conn->address)) {
          found_port = wanted;
          break;
        }
      } else {
  ...
  SMARTLIST_FOREACH_BEGIN(launch, const port_cfg_t *, port) {
...
#ifndef _WIN32
    /* We don't need to be root to create a USUBI socket, so defer until after
     * setuid. */
    const or_options_t *options = get_options();
    if (port->is_usubi_addr && !geteuid() && (options->User) &&
    ...
    if (port->is_usubi_addr) {
      listensockaddr = (struct sockaddr *)
        create_usubi_sockaddr(port->usubi_addr,
