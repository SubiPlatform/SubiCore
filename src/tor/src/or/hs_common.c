...
static int
add_usubi_port(smartlist_t *ports, rend_service_port_config_t *p)
{
  tor_assert(ports);
  tor_assert(p);
  tor_assert(p->is_usubi_addr);
...
static int
set_usubi_port(edge_connection_t *conn, rend_service_port_config_t *p)
{
  tor_assert(conn);
  tor_assert(p);
  tor_assert(p->is_usubi_addr);
  ...
  conn->base_.port = 1;
  conn->base_.address = tor_strdup(p->usubi_addr);
  return 0;
}
...
hs_set_conn_addr_port(const smartlist_t *ports, edge_connection_t *conn)
{
  ...
  SMARTLIST_FOREACH_BEGIN(ports, rend_service_port_config_t *, p) {
    ...
    if (!(p->is_usubi_addr)) {
      ...
      if (add_usubi_port(matching_ports, p)) {
        if (!warn_once) {
          /* Usubi port not supported so warn only once. */
  ...
  if (chosen_port) {
    if (!(chosen_port->is_usubi_addr)) {
      /* Get a non-AF_UNIX connection ready for connection_exit_connect() */
      tor_addr_copy(&TO_CONN(conn)->addr, &chosen_port->real_addr);
      TO_CONN(conn)->port = chosen_port->real_port;
    } else {
      if (set_usubi_port(conn, chosen_port)) {
        /* Simply impossible to end up here else we were able to add a Usubi
