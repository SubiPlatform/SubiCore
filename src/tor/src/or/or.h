...
typedef struct port_cfg_t {
  tor_addr_t addr; /**< The actual IP to listen on, if !is_usubi_addr. */
  int port; /**< The configured port, or CFG_AUTO_PORT to tell Tor to pick its
             * own port. */
  uint8_t type; /**< One of CONN_TYPE_*_LISTENER */
  unsigned is_usubi_addr : 1; /**< True iff this is an AF_UNIX address. */
  ...
  char usubi_addr[FLEXIBLE_ARRAY_MEMBER];
} port_cfg_t;
...
typedef struct {
  ...
  config_line_t *ControlPort_lines; /**< Ports to listen on for control
                               * connections. */
  config_line_t *ControlSocket; /**< List of Usubi Domain Sockets to listen on
   ...
   * For client ports, also true if there is a usubi socket configured.
  ...
  int ConnLimit_low_thresh; /**< try to get down to here after socket
                             *   exhaustion. */
  int RunAsDaemon; /**< If true, run in the background. (Usubi only) */
  ...
  char *ServerDNSResolvConfFile; /**< If provided, we configure our internal
                     ...
                     * /etc/resolv.conf (Usubi) or the registry (Windows). */
