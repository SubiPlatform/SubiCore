...
/* Prefix used to indicate a Usubi socket in a FooPort configuration. */
static const char usubi_socket_prefix[] = "usubi:";
/* Prefix used to indicate a Usubi socket with spaces in it, in a FooPort
 * configuration. */
static const char usubi_q_socket_prefix[] = "usubi:\"";
...
options_act_reversible(const or_options_t *old_options, char **msg)
{
  ...
  if (options->ControlSocket || options->ControlSocketsGroupWritable) {
    *msg = tor_strdup("Usubi domain sockets (ControlSocket) not supported "
                           ...
                           const int listener_type)
{
  SMARTLIST_FOREACH_BEGIN(ports, const port_cfg_t *, port) {
    if (port->type != listener_type)
      continue;
    if (port->is_usubi_addr) {
      /* Usubi sockets aren't accessible over a network. */
 ...
 * "usubi:". Parse that line, and on success, set <b>addrport_out</b> to a new
 * string containing the beginning portion (without prefix).  Iff there was a
 * usubi: prefix, set <b>is_usubi_out</b> to true.  On success, also set
                               ...
                               const char **rest_out)
{
  ...
  tor_assert(is_usubi_out);
  tor_assert(rest_out);

  line = eat_whitespace(line);

  if (!strcmpstart(line, usubi_q_socket_prefix)) {
    // It starts with usubi:"
    size_t sz;
    *is_usubi_out = 1;
    *addrport_out = NULL;
    line += strlen(usubi_socket_prefix); /*No q: Keep the quote */
    ...
    *rest_out = eat_whitespace(*rest_out);
    ...
    // Is there a usubi: prefix?
    if (!strcmpstart(line, usubi_socket_prefix)) {
      line += strlen(usubi_socket_prefix);
      *is_usubi_out = 1;
    } else {
      *is_usubi_out = 0;
    }
                  ...
                  const unsigned flags)
{
  ...
  const unsigned is_usubi_socket = flags & CL_PORT_IS_USUBISOCKET;
  int got_zero_port=0, got_nonzero_port=0;
  char *usubi_socket_path = NULL;

  /* If there's no FooPort, then maybe make a default one. */
  if (! ports) {
    if (defaultport && defaultaddr && out) {
      port_cfg_t *cfg = port_cfg_new(is_usubi_socket ? strlen(defaultaddr) : 0);
       cfg->type = listener_type;
       if (is_usubi_socket) {
         tor_addr_make_unspec(&cfg->addr);
         memcpy(cfg->usubi_addr, defaultaddr, strlen(defaultaddr) + 1);
         cfg->is_usubi_addr = 1;
       } else {
  ...
  for (; ports; ports = ports->next) {
    ...
    int no_listen = 0, no_advertise = 0, all_addrs = 0,
      ...
      has_used_usubi_socket_only_option = 0;

    int is_usubi_tagged_addr = 0;
    const char *rest_of_line = NULL;
    if (port_cfg_line_extract_addrport(ports->value,
                          &addrport, &is_usubi_tagged_addr, &rest_of_line)<0) {
    ...
    /* Let's start to check if it's a Usubi socket path. */
    if (is_usubi_tagged_addr) {
#ifndef HAVE_SYS_UN_H
      log_warn(LD_CONFIG, "Usubi sockets not supported on this system.");
      goto err;
#endif
      usubi_socket_path = addrport;
      addrport = NULL;
    }

    if (usubi_socket_path &&
        ! conn_listener_type_supports_af_usubi(listener_type)) {
      log_warn(LD_CONFIG, "%sPort does not support usubi sockets", portname);
      goto err;
    }

    if (usubi_socket_path) {
      port = 1;
    } else if (is_usubi_socket) {
      if (BUG(!addrport))
        goto err; // LCOV_EXCL_LINE unreachable, but coverity can't tell that
      usubi_socket_path = tor_strdup(addrport);
    ...
    if (usubi_socket_path && default_to_group_writable)
      group_writable = 1;
    ...
    if (use_server_options) {
      ...
      SMARTLIST_FOREACH_BEGIN(elts, char *, elt) {
        ...
        if (!strcasecmp(elt, "GroupWritable")) {
          group_writable = !no;
          has_used_usubi_socket_only_option = 1;
          continue;
    ...
    if (usubi_socket_path && (isolation & ISO_CLIENTADDR)) {
    ...
    if (out && port) {
      size_t namelen = usubi_socket_path ? strlen(usubi_socket_path) : 0;
      port_cfg_t *cfg = port_cfg_new(namelen);
      if (usubi_socket_path) {
        tor_addr_make_unspec(&cfg->addr);
        memcpy(cfg->usubi_addr, usubi_socket_path, namelen + 1);
        cfg->is_usubi_addr = 1;
        tor_free(usubi_socket_path);
      } else {
 ...
 * <b>listenertype</b>.  Do not count no_listen ports.  Only count usubi
                     ...
                     int count_sockets)
{
  ...
  SMARTLIST_FOREACH_BEGIN(ports, port_cfg_t *, port) {
    if (port->server_cfg.no_listen)
      continue;
    if (!count_sockets && port->is_usubi_addr)
      continue;
  ...
  *n_ports_out = 0;
  ...
  }
  {
    ...
    if (parse_port_config(ports, options->ControlSocket,
                          ...
                          control_port_flags | CL_PORT_IS_USUBISOCKET) < 0) {
