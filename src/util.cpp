...
boost::filesystem::path GetSubinodeConfigFile()
{
    boost::filesystem::path pathConfigFile(gArgs.GetArg("-subinodeconf", "subinode.conf"));
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\subi
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\subi
    // Mac: ~/Library/Application Support/subi
    // Usubi: ~/.subi
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Subi";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Subi";
#else
    // Usubi
    return pathRet / ".subi";
#endif
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good())
        return; // No subi.conf file is OK

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override subi.conf
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Subi Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Subi Core developers";
    }
