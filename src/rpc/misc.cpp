...
public:
    ...
    UniValue operator()(const CShadeAddress &sxAddr) const {
        UniValue obj(UniValue::VOBJ);
        obj.push_back(Pair("isshadeaddress", true));
        obj.push_back(Pair("prefix_num_bits", sxAddr.prefix.number_bits));
        obj.push_back(Pair("prefix_bitfield", strprintf("0x%04x", sxAddr.prefix.bitfield)));
        return obj;
    }
...
//Subinode
UniValue subinodesync(const JSONRPCRequest& req)
{
    UniValue params = req.params;
    bool fHelp = req.fHelp;
    if (fHelp || params.size() != 1)
        throw runtime_error(
                "subinodesync [status|next|reset]\n"
    ...
    if(strMode == "status") {
        ...
        objStatus.push_back(Pair("AssetID", subinodeSync.GetAssetID()));
        objStatus.push_back(Pair("AssetName", subinodeSync.GetAssetName()));
        objStatus.push_back(Pair("Attempt", subinodeSync.GetAttempt()));
        objStatus.push_back(Pair("IsBlockchainSynced", subinodeSync.IsBlockchainSynced()));
        objStatus.push_back(Pair("IsSubinodeListSynced", subinodeSync.IsSubinodeListSynced()));
        objStatus.push_back(Pair("IsWinnersListSynced", subinodeSync.IsWinnersListSynced()));
        objStatus.push_back(Pair("IsSynced", subinodeSync.IsSynced(chainActive.Height())));
        objStatus.push_back(Pair("IsFailed", subinodeSync.IsFailed()));
        return objStatus;
    }

    if(strMode == "next")
    {
        subinodeSync.SwitchToNextAsset();
        return "sync updated to " + subinodeSync.GetAssetName();
    }
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "to the wallet before upgrading to v0.17. To use the deprecated functionality, start subid with -deprecatedrpc=createmultisig\n"
            "\nArguments:\n"
    ...
    for (unsigned int i = 0; i < keys.size(); ++i) {
        if (IsHex(keys[i].get_str()) && (keys[i].get_str().length() == 66 || keys[i].get_str().length() == 130)) {
            ...
            " To use the deprecated functionality, start subid with -deprecatedrpc=createmultisig", keys[i].get_str()));
        }
    }
...
UniValue setmocktime(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. timestamp  (integer, required) Usubi seconds-since-epoch timestamp\n"
