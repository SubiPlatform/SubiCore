...
#include "subinode/netfulfilledman.h"
#include "subinode/instantx.h"
#include "subinode/spork.h"
#include "subinode/flat-database.h"
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/subiplatform/subicore>";
    const std::string URL_WEBSITE = "<https://subiplatform.io>";
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if SUBI is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if SUBI is started while in a temporary directory.\n",
    ...
    boost::filesystem::path pathVersionSetting = GetDataDir()/"subiversion.dat";
    ...
    LogPrintf("AppInitMain(): Wallet version %s logged to subiversion.dat", versionArg.second);
    ...
    fSubiNode = gArgs.GetBoolArg("-subinode", false);

    LogPrintf("fSubiNode = %s\n", fSubiNode);
    LogPrintf("subinodeConfig.getCount(): %s\n", subinodeConfig.getCount());

    if ((fSubiNode || subinodeConfig.getCount() > 0) && !fTxIndex) {
        return InitError("Enabling Subinode support requires turning on transaction indexing."
    ...
    if (fSubiNode) {
        ...
        if (!gArgs.GetArg("-subinodeaddr", "").empty()) {
            // Hot Subinode (either local or remote) should get its address in
            // CActiveSubinode::ManageState() automatically and no longer relies on Subinodeaddr.
            return InitError(_("subinodeaddr option is deprecated. Please use subinode.conf to manage your remote subinodes."));
        }

        std::string strSubinodePrivKey = gArgs.GetArg("-subinodeprivkey", "");
        if (!strSubinodePrivKey.empty()) {
            if (!darkSendSigner.GetKeysFromSecret(strSubinodePrivKey, activeSubinode.keySubinode,
                                                  activeSubinode.pubKeySubinode))
                return InitError(_("Invalid subinode privkey. Please see documenation."));

            LogPrintf("pubKeySubinode: %s\n", CBitcoinAddress(activeSubinode.pubKeySubinode.GetID()).ToString());
        } else {
            return InitError(
                        _("You must specify a subinode privkey in the configuration. Please see documentation for help."));
        }
    }

    LogPrintf("Using Subinode config file %s\n", GetSubinodeConfigFile().string());

    if (gArgs.GetBoolArg("-subinodeconflock", true) && vpwallets.front() && (subinodeConfig.getCount() > 0)) {
        LOCK(vpwallets.front()->cs_wallet);
        LogPrintf("Locking Subinodes:\n");
        uint256 mnTxHash;
        int outputIndex;
    ...
    //lite mode disables all Subinode and Darksend related functionality
    fLiteMode = gArgs.GetBoolArg("-litemode", false);
    if (fSubiNode && fLiteMode) {
        return InitError("You can not start a subinode in litemode");
    }
    ...
    // ********************************************************* Step 11c: update block tip in subi modules


    mnodeman.UpdatedBlockTip(chainActive.Tip());
    darkSendPool.UpdatedBlockTip(chainActive.Tip());
    mnpayments.UpdatedBlockTip(chainActive.Tip());
    subinodeSync.UpdatedBlockTip(chainActive.Tip());

    // ********************************************************* Step 11d: start subinode thread
    ...
    //do not allow subinodes to run staking threads to avoid bandwidth issues
    if(!fSubiNode){
        #ifdef ENABLE_WALLET
        nMinStakeInterval = gArgs.GetArg("-minstakeinterval", 0);
        nMinerSleep = gArgs.GetArg("-minersleep", 500);
        if (!gArgs.GetBoolArg("-staking", true))
            LogPrintf("SUBI Staking disabled\n");
        else
        {
