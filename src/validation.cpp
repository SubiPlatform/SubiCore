...
#if defined(NDEBUG)
# error "SUBI cannot be compiled without assertions."
#endif
...
//TODO: configure subinode fees
CAmount GetSubinodePayment(int nHeight, CAmount blockValue) {

    const Consensus::Params &consensusParams = Params().GetConsensus();
    CAmount ret = GetBlockSubsidy(nHeight, consensusParams) * SUBINODE_REWARD;

    //Switch node subsidy upon PoS upgrade
    if(nHeight >= consensusParams.nPosHeightActivate)
        ret = GetBlockSubsidy(nHeight, consensusParams) * SUBINODE_REWARD_POST_POS;
...
const std::string strMessageMagic = "SUBI Signed Message:\n";
...
bool GetSubinodeFeePayment(int64_t &returnFee, bool &payFees, const CBlock &pBlock){

    CAmount totalShaded = 0;
    vector<CAmount> mintVector;
    mintVector.clear();
    if(chainActive.Height() + 1 >= Params().GetConsensus().nStartShadeFeeDistribution){
        //Time to payout all subinodes and check
        //LogPrintf("\nGetSubinodeFeePayment(): height=%d, modulo=%d\n",
        //          (chainActive.Height() + 1), ((chainActive.Height() + 1) % Params().GetConsensus().nShadeFeeDistributionCycle));
        if(((chainActive.Height() + 1) % Params().GetConsensus().nShadeFeeDistributionCycle) == 0){
            //Subtract 1 from sample since we check current block fees
            int sample = Params().GetConsensus().nShadeFeeDistributionCycle - 1;
            mintVector.clear();
            ...
            returnFee = totalShaded * 0.0025;
            payFees = true;
            return true;
        }
        //Make sure all Shade fees in this block are not paid out
        else{
            for(auto ctx: pBlock.vtx){
                //Found Shade fee transaction
                if(!ctx->IsZerocoinSpend() && ctx->IsZerocoinMint()){
...
bool CheckShadeProtocolFeePayouts(const CBlock &pBlock, int64_t &totalFees){
    ...
    if(subinodeSync.IsSynced(chainActive.Height())){
        ...
        vector<CScript> subinodeVectorWinners;
        subinodeVectorWinners.clear();
        ...
        for(auto node: subinodeVector){
            ...
            if(node.IsEnabled() && (node.sigTime >= ensureNodeActiveBefore)){
                ...
                subinodeVectorWinners.push_back(nodeScript);
            }
        }
        ...
        for(CTxOut out: pBlock.vtx[0]->vout){
            if((std::find(subinodeVectorWinners.begin(), subinodeVectorWinners.end(), out.scriptPubKey) != subinodeVectorWinners.end()) && out.nValue == feePayout) {
                //erase this check to prevent duplicate payouts
                subinodeVectorWinners.erase(std::find(subinodeVectorWinners.begin(), subinodeVectorWinners.end(), out.scriptPubKey));
                totalNodesPaid++;
            }
        }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (block.IsProofOfStake()) // check payment information on staked blocks
    {
        ...
        if(!GetSubinodeFeePayment(returnFee, payFees, block))
            return state.DoS(100, error("ConnectBlock() : GetSubinodeFeePayment incorrect Shade fee scheduling."), REJECT_INVALID, "bad-cs-amount");
        ...
        BOOST_FOREACH(const CTxOut &output, txCoinstake->vout) {
            if (output.scriptPubKey == DEV_SCRIPT && output.nValue == (int64_t)(DEVELOPMENT_REWARD_POST_POS * GetBlockSubsidy(nHeight, Params().GetConsensus()))) {
                found_dev = true;
            }
        }
        ...
        if (!(found_dev)) {
            nCalculatedStakeReward = Params().GetProofOfStakeReward(pindex->pprev, nFees) +
                ((SUBINODE_REWARD_POST_POS) * GetBlockSubsidy(pindex->nHeight, Params().GetConsensus()));
            }
        }
        ...
        //Payout fees for subinode fee cycle
        else{
            /*
            if(!CheckShadeProtocolFeePayouts(block, returnFee))
                return state.DoS(100, error("CheckShadeProtocolFeePayouts() : block does not payout correct subinode fees"), REJECT_INVALID, "bad-cs-amount");
            */
            blockReward = blockReward + returnFee;
        }
        ...
        found_dev = false;
        CAmount subinodeReward = (int64_t)(SUBINODE_REWARD_POST_POS * GetBlockSubsidy(nHeight, Params().GetConsensus()));
        //check subinode payout,
        if(chainActive.Height() + 1 < Params().GetConsensus().nSubinodePaymentsStartBlock || (mnodeman.GetFullSubinodeVector().size() < 10)){
            found_dev = true;
        }
        else{
            BOOST_FOREACH(const CTxOut &output, txCoinstake->vout) {
                //check that subinode reward at least the blockreward, accounts for Shade fees
                if (output.nValue >= subinodeReward) {
                    found_dev = true;
                }
            }
        }
        ...
        if(!found_dev){
            return state.DoS(100, false, REJECT_FOUNDER_REWARD_MISSING,
                             "CTransaction::CheckTransaction() : subinode reward missing");
        }
    ...
    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward)) {
        ...
        return state.DoS(0, error("ConnectBlock(): couldn't find subinode payments"),
    ...
    if (!ConnectBlockShade(state, chainparams, pindex, &block))
        return false;
    ...
    if(!pindex->pprev->IsProofOfStake() && pindex->IsProofOfStake()){
        ...
        //LogPrintf("SUBI Money Supply: %llf", pindex->nMoneySupply/COIN);
    }
...
void static UpdateTip(const CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    subinodeSync.UpdatedBlockTip(pindexNew);
...
bool CChainState::DisconnectTip(CValidationState& state, const CChainParams& chainparams, DisconnectedBlockTransactions *disconnectpool)
{
    ...
    DisconnectTipShade(block, pindexDelete);
...
bool ProcessNewBlock(const CChainParams& chainparams, const std::shared_ptr<const CBlock> pblock, bool fForceProcessing, bool *fNewBlock)
{
    ...
    subinodeSync.IsBlockchainSynced(true);
