...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    // Subinode Related Inventory Messages
    ...
    case MSG_SUBINODE_PAYMENT_VOTE:
        return mnpayments.mapSubinodePaymentVotes.count(inv.hash);

    case MSG_SUBINODE_PAYMENT_BLOCK:
    {
        BlockMap::iterator mi = mapBlockIndex.find(inv.hash);
        return mi != mapBlockIndex.end() && mnpayments.mapSubinodeBlocks.find(mi->second->nHeight) != mnpayments.mapSubinodeBlocks.end();
    }

    case MSG_SUBINODE_ANNOUNCE:
        return mnodeman.mapSeenSubinodeBroadcast.count(inv.hash) && !mnodeman.IsMnbRecoveryRequested(inv.hash);

    case MSG_SUBINODE_PING:
        return mnodeman.mapSeenSubinodePing.count(inv.hash);

    case MSG_DSTX:
        return mapDarksendBroadcastTxes.count(inv.hash);

    case MSG_SUBINODE_VERIFY:
        return mnodeman.mapSeenSubinodeVerification.count(inv.hash);
    }
    ...
    const CNetMsgMaker msgMaker(pfrom->GetSendVersion());
    {
        ...
        while (it != pfrom->vRecvGetData.end() && !(it->type == MSG_BLOCK || it->type == MSG_FILTERED_BLOCK || it->type == MSG_CMPCT_BLOCK || it->type == MSG_WITNESS_BLOCK)) {
            ...
            if((it->type == MSG_TX || it->type == MSG_WITNESS_TX)){
                ...
                if (!pushed && inv.type == MSG_SUBINODE_PAYMENT_VOTE) {
                    if(mnpayments.HasVerifiedPaymentVote(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnpayments.mapSubinodePaymentVotes[inv.hash];
                        const CNetMsgMaker msgMaker(pfrom->GetSendVersion());
                        connman->PushMessage(pfrom, msgMaker.Make(NetMsgType::SUBINODEPAYMENTVOTE, ss));
                        pushed = true;
                    }
                }

                if (!pushed && inv.type == MSG_SUBINODE_PAYMENT_BLOCK) {
                    BlockMap::iterator mi = mapBlockIndex.find(inv.hash);
                    LOCK(cs_mapSubinodeBlocks);
                    if (mi != mapBlockIndex.end() && mnpayments.mapSubinodeBlocks.count(mi->second->nHeight)) {
                        BOOST_FOREACH(CSubinodePayee& payee, mnpayments.mapSubinodeBlocks[mi->second->nHeight].vecPayees) {
                            std::vector<uint256> vecVoteHashes = payee.GetVoteHashes();
                            BOOST_FOREACH(uint256& hash, vecVoteHashes) {
                                if(mnpayments.HasVerifiedPaymentVote(hash)) {
                                    CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                                    ss.reserve(1000);
                                    ss << mnpayments.mapSubinodePaymentVotes[hash];
                                    const CNetMsgMaker msgMaker(pfrom->GetSendVersion());
                                    connman->PushMessage(pfrom, msgMaker.Make(NetMsgType::SUBINODEPAYMENTVOTE, ss));
                                }
                            }
                ...
                if (!pushed && inv.type == MSG_SUBINODE_ANNOUNCE) {
                    if(mnodeman.mapSeenSubinodeBroadcast.count(inv.hash)){
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenSubinodeBroadcast[inv.hash].second;
                        const CNetMsgMaker msgMaker(pfrom->GetSendVersion());
                ...
                if (!pushed && inv.type == MSG_SUBINODE_PING) {
                    if(mnodeman.mapSeenSubinodePing.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenSubinodePing[inv.hash];
                        const CNetMsgMaker msgMaker(pfrom->GetSendVersion());
                ...
                if (!pushed && inv.type == MSG_SUBINODE_VERIFY) {
                    if(mnodeman.mapSeenSubinodeVerification.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenSubinodeVerification[inv.hash];
                        const CNetMsgMaker msgMaker(pfrom->GetSendVersion());
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::VERSION)
    {
        ...
        static const int FORK_MIN_PEER_PROTO_VERSION = nHeight >= Params().GetConsensus().nStartShadeFeeDistribution ? MIN_PEER_DPOS_PROTO_VERSION : MIN_PEER_PROTO_VERSION;
    ...
    else if (strCommand == NetMsgType::TX || strCommand == NetMsgType::DSTX || strCommand == NetMsgType::TXLOCKREQUEST || strCommand == "witness-tx")
    {
        ...
        if (strCommand == NetMsgType::TXLOCKREQUEST) {
            ...
            CSubinode *pmn = mnodeman.Find(dstx.vin);
            if (pmn == NULL) {
                LogPrintf("privatesend", "DSTX -- Can't find subinode %s to verify %s\n",
                         dstx.vin.prevout.ToStringShort(), hashTx.ToString());
                return false;
            }

            if (!pmn->fAllowMixingTx) {
                LogPrintf("privatesend", "DSTX -- Subinode %s is sending too many transactions %s\n",
                         dstx.vin.prevout.ToStringShort(), hashTx.ToString());
                return true;
            ...
            if (!dstx.CheckSignature(pmn->pubKeySubinode)) {
    ...
    else if (strCommand == NetMsgType::FILTERCLEAR)
    {
        ...
        if (found) {
            ...
            subinodeSync.ProcessMessage(pfrom, strCommandNonConst, vRecv);
        } else {
            // Ignore unknown commands for extensibility
