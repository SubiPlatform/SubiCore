...
#include <shade-address/extkey.h>
...
public:
    ...
    bool GetSubinodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash = "", std::string strOutputIndex = "");
    ...
    CAmount GetShadeBalance() const;
    CAmount GetShadeBalanceUnconfirmed() const;
    ...
    bool FindUnloadedShadeTransactions(const CTransaction& tx);
     ...
     * Add Shade functions
     */
    bool EnableShadeMode(SecureString strWalletPass,string totalAmount);
    bool DisableShadeMode();
    bool ShadeModeMintTrigger(string totalAmount, vector<CScript> pubCoinScripts = vector<CScript>());
    std::string ShadeModeSpendTrigger(string denomination, string toKey = "", vector<CScript> pubCoinScripts = vector<CScript>());
    ...
    bool CreateCoinStake(unsigned int nBits, int64_t nTime, int nBlockHeight, int64_t nFees, CMutableTransaction &txNew, CKey &key, CBlockTemplate* pblocktemplate, int64_t nShadeFees, std::vector<unsigned char> &commitment, uint256 witnessroot);
    ...
    void NotifyShadeChanged(CWallet *wallet, const std::string &pubCoin, int denomination, const std::string &isUsed, ChangeType status);
