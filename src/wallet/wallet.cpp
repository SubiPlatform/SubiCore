...
#include "subinode/darksend.h"
#include "subinode/instantx.h"
#include "subinode/subinode.h"
#include "subinode/subinode-payments.h"
...
public:
    ...
    void operator()(const CShadeAddress &stxAddr) {
        CScript script;
    }
...
bool CWallet::AddToWalletIfInvolvingMe(const CTransactionRef& ptx, const CBlockIndex* pIndex, int posInBlock, bool fUpdate)
{
    const CTransaction& tx = *ptx;
    {
        ...
        bool foundZerocoin = FindUnloadedShadeTransactions(tx);
...
CAmount CWallet::GetShadeBalance() const
{
...
CAmount CWallet::GetShadeBalanceUnconfirmed() const
{
    std::vector<COutput> vCoins;
        ...
        for (const auto& entry : mapWallet)
        {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                if (nCoinType == ONLY_DENOMINATED) {
                    ...
                    found = !(fSubiNode && pcoin->tx->vout[i].nValue == SUBINODE_COIN_REQUIRED * COIN);
                } else if (nCoinType == ONLY_NONDENOMINATED_NOT10000IFMN) {
                    if (IsCollateralAmount(pcoin->tx->vout[i].nValue)) continue; 
                    found = !IsDenominatedAmount(pcoin->tx->vout[i].nValue);
                    if (found && fSubiNode) found = pcoin->tx->vout[i].nValue != SUBINODE_COIN_REQUIRED * COIN; 
                } else if (nCoinType == ONLY_10000) {
                    found = pcoin->tx->vout[i].nValue == SUBINODE_COIN_REQUIRED * COIN;
                    isGN = true;
                ...
                if (pick_new_inputs) {
                    ...
                    if (!SelectCoins(vAvailableCoins, nValueToSelect, setCoins, nValueIn, &coin_control))
                    {
                        if (nCoinType == ONLY_NOT10000IFMN) {
                            strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 SUBI.");
                        } else if (nCoinType == ONLY_NONDENOMINATED_NOT10000IFMN) {
                            strFailReason = _("Unable to locate enough PrivateSend non-denominated funds for this transaction that are not equal 10000 SUBI.");
...
{
    FindUnloadedShadeTransactions(*wtxNew.tx);
    {
        LOCK2(cs_main, cs_wallet);
...
static const std::string OUTPUT_TYPE_STRING_SUBINODE = "subinode";
...
OutputType ParseOutputType(const std::string& type, OutputType default_type)
{
    ...
    }  else if (type == OUTPUT_TYPE_STRING_SUBINODE) {
        return OUTPUT_TYPE_LEGACY;
    } else {
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                //add 0.25% tx fee to all Shade transaction
...
//            LogPrintf("txNew:%s\n", txNew.ToString());
            ...
            if (coinId == INT_MAX){
                strFailReason = _("network privacy set too low. There needs to be at least 2 shaded values of this type! ");
                return false;
            }
...
bool CWallet::CreateZerocoinSpendTransactionBatch(std::string &toKey, vector <CScript> pubCoinScripts, vector <int64_t> nValueBatch, vector <libzerocoin::CoinDenomination> denominationBatch,
    ...
    if(pubCoinScripts.empty()){
        ...
        else if (IsShadeAddress(toKey))
        {
            CShadeAddress sxAddr;
            if (sxAddr.SetEncoded(toKey))
            {
                ...
                shadeKey = CScript() << OP_RETURN << ephem_pubkey;
    ...
    for(int i = 0; i < nValueBatch.size(); i++){
        {
            LOCK2(cs_main, cs_wallet);
            {
                ...
                if (coinId == INT_MAX){
                    strFailReason = _("network privacy set too low. There needs to be at least 2 shaded values of this type!")  + std::to_string(coinHeight) + " " + std::to_string(coinId);
                    return false;
                }
...
//unlock wallet and create shade timer
bool CWallet::EnableShadeMode(SecureString strWalletPass, string totalAmount){

    this->NotifyZerocoinChanged.connect(boost::bind(&CWallet::NotifyShadeChanged, this, _1, _2, _3, _4, _5));

    //sanity check for decimal
    if (totalAmount.find('.') != std::string::npos)
        return error("%s: Error: The Shade Mode value needs to be a whole number.", __func__);
    ...
    if(!ShadeModeMintTrigger(totalAmount))
        return error("%s: Error: Cannot trigger shade mode mint.", __func__);

    return true;
}

//Lock wallet and destroy shade timer
bool CWallet::DisableShadeMode(){
    LOCK(this->cs_wallet);
    this->nRelockTime = 0;
    this->NotifyZerocoinChanged.disconnect(boost::bind(&CWallet::NotifyShadeChanged, this, _1, _2, _3, _4, _5));
    this->Lock();
    return true;
}

//push mint and spends into a queue
void CWallet::NotifyShadeChanged(CWallet *wallet, const std::string &pubCoin, int denomination, const std::string &isUsed, ChangeType status)
{
    ...
    if(isUsed == "New")
        ShadeModeSpendTrigger(std::to_string(denom));
    if(isUsed == "Used")
        ShadeModeMintTrigger(std::to_string(denom));
    ...
    BOOST_FOREACH(const CZerocoinEntry &minIdPubcoin, pc) {
        if(minIdPubcoin.IsUsed == false){
            ...
            if(coinHeight >= chainActive.Height() + ZEROCOIN_CONFIRM_HEIGHT){
                //if(!ShadeModeSpendTrigger(std::to_string(minIdPubcoin.denomination)))
...
//shade timer mint responder
bool CWallet::ShadeModeMintTrigger(string totalAmount, vector<CScript> pubCoinScripts){

    string stringError;
    //Autobackup wallet into shadebackups
    string backupDir = GetDataDir().string() + "/shadebackups/wallet-" + std::to_string(GetTime()) + ".dat";
    fs::create_directories(fs::path(GetDataDir().string() + "/shadebackups/"));

    //only maintain max of 20 backups
    std::string path = GetDataDir().string() + "/shadebackups/";
    int m = 0;
    int t = 0;
...
bool ClosestDenoms(CAmount amount, int totalZerocoins, CAmount demoniationList[8][1]){
    ...
    for (int i = 0; i < totalZerocoins; i++) {
        ...
        if (amount == currentDenomination[currentDenominationIndex]) {
            ...
            if(demoniationList[ShadeDenom(currentDenomination[currentDenominationIndex])][0] > 0){
                demoniationList[ShadeDenom(currentDenomination[currentDenominationIndex])][0]--;
                amount = 0;
            }
            i = totalZerocoins;
            break;
...
//shade timer spend responder
std::string CWallet::ShadeModeSpendTrigger(string totalAmount, string toKey, vector<CScript> pubCoinScripts){
    ...
    if(!this->GetDBHandle().Backup(backupDir))
        return "ShadeModeSpendTrigger(): Error: Cannot create wallet backup.";
    ...
    CAmount amount = 0;
    if (!ParseFixedPoint(totalAmount, 8, &amount))
        return "ShadeModeSpendTrigger(): Error: Invalid amount.";
    if (!MoneyRange(amount))
        return "ShadeModeSpendTrigger(): Error: Amount out of range.";
    ...
    for (int i = 0; i <= totalZerocoins; i++) {
        ...
        if (amount == currentDenomination[currentDenominationIndex]) {
            //check if we have this denomination
            if(demoniationList[ShadeDenom(currentDenomination[currentDenominationIndex])][0] > 0){
                toSpend[ShadeDenom(currentDenomination[currentDenominationIndex])][0]++;
                demoniationList[ShadeDenom(currentDenomination[currentDenominationIndex])][0]--;
                amount = 0;
                i = totalZerocoins;
                break;
    ...
    if(amount == 0){
        ...
        if (this->IsLocked())
            return "ShadeModeSpendTrigger(): Error: The wallet needs to be unlocked.";

        //check if minting these spends
        if(!pubCoinScripts.empty()){
            //Not enough payout scripts
            if(pubCoinScripts.size() < denominationBatch.size())
                return "ShadeModeSpendTrigger(): Error: Not enough mint payout scripts "
            ...
            for(int ps = 0; ps < pubCoinScripts.size(); ps++){
                ...
                CBigNum pubCoin(vector<unsigned char>(pubCoinScripts[ps].begin()+6, pubCoinScripts[ps].end()));
                if (!zcState->CanAddMintToMempool(pubCoin)) {
                    return "ShadeModeSpendTrigger(): Error: key has already been used!";
                }
            }
        ...
        for(int vecSplit = 0; vecSplit < ((denominationBatch.size()/4) + 1); vecSplit++){
            ...
            if(!CreateZerocoinSpendModelBatch(stringError, denominationBatchSub, toKey, pubCoinScriptsSub)){
                if(vecSplit > 0){
                    CAmount amountShaded = 0;
                    for (int x = 0; x < vecSplit; x++){
                        amountShaded += currentDenomination[x];
                        amountShaded += currentDenomination[x + 1];
                        amountShaded += currentDenomination[x + 2];
                        amountShaded += currentDenomination[x + 3];
                    }
                    return "ShadeModeSpendTrigger(): Error: Was only able to unshade %s SUBI - %s." + std::to_string(amountShaded) + stringError;
                }
                else
                    return "ShadeModeSpendTrigger(): Error: Failed to unshade shaded SUBI - %s." +  stringError;
            }
        ...
        return "Sucessfully sent " + totalAmount + " shaded SUBI";
    }
    else {
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector <CTxIn> &vecTxInRet, CAmount &nValueRet,
    ...
    vector <COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, false, MAX_MONEY, MAX_MONEY,0, 0, 9999999, nPrivateSendRoundsMin < 0 ? ONLY_NONDENOMINATED_NOT10000IFMN : ONLY_DENOMINATED);
    ...
    BOOST_FOREACH(const COutput &out, vCoins)
    {
        ...
        if (fSubiNode && out.tx->tx->vout[out.i].nValue == SUBINODE_COIN_REQUIRED * COIN) continue; //subinode input
...
bool CWallet::GetSubinodeVinAndKeys(CTxIn &txinRet, CPubKey &pubKeyRet, CKey &keyRet, std::string strTxHash,
    ...
    AvailableCoins(vPossibleCoins, true, NULL, false, MAX_MONEY, MAX_MONEY,0, 0, 9999999, ONLY_10000);

    if (vPossibleCoins.empty()) {
        LogPrintf("CWallet::GetSubinodeVinAndKeys -- Could not locate any valid subinode vin\n");
        return false;
    }
    ...
    LogPrintf("CWallet::GetSubinodeVinAndKeys -- Could not locate specified subinode vin\n");
    return false;
}
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax,
    ...
    BOOST_FOREACH(const COutput &out, vCoins)
    {
        // subinode-like input should not be selected by AvailableCoins now anyway
        if (nValueRet + out.tx->tx->vout[out.i].nValue <= nValueMax) {
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
        ...
        for (unsigned int i = 0; i < wtx.tx->vout.size(); i++) {
            ...
            if (fAnonymizable) {
                ...
                if (fSubiNode && wtx.tx->vout[i].nValue == SUBINODE_COIN_REQUIRED * COIN) continue;
...
size_t CWallet::CountColdstakeOutputs()
{
    ...
    AvailableCoins(vAvailableCoins, false, &coinControl, nMinimumAmount, nMaximumAmount, nMinimumSumAmount, nMaximumCount, nMinDepth, nMaxDepth, ONLY_NONDENOMINATED_NOT10000IFMN);
...
void CWallet::AvailableCoinsForStaking(std::vector<COutput> &vCoins, int64_t nTime, int nHeight) const
{
...

    {
        ...
        int coinbaseMaturity = nHeight >= Params().GetConsensus().nStartShadeFeeDistribution ? COINBASE_MATURITY_V2 : COINBASE_MATURITY;
...
bool CWallet::CreateCoinStake(unsigned int nBits, int64_t nTime, int nBlockHeight, int64_t nFees, CMutableTransaction &txNew, CKey &key, CBlockTemplate *pblocktemplate, int64_t nShadeFees, std::vector<unsigned char> &commitment, uint256 witnessroot)
{
    ...
    for (; it != setCoins.end(); ++it)
    {
        ...
        if (CheckKernel(pindexPrev, nBits, nTime, prevoutStake, &nBlockTime))
        {
            ...
            */
            ...
            txNew.nVersion = SUBI_TXN_VERSION;
    ...
    txNew.vout.push_back(CTxOut(DEVELOPMENT_REWARD_POST_POS * GetBlockSubsidy(chainActive.Height(), Params().GetConsensus()), CScript(DEV_SCRIPT.begin(), DEV_SCRIPT.end())));


    CBlock *pblock = &pblocktemplate->block; // pointer for convenience


    //payout 10 subinodes for rewards
    if(chainActive.Height() + 1 < Params().GetConsensus().nStartShadeFeeDistribution){
        if(nShadeFees > 0 && chainActive.Height() >= Params().GetConsensus().nSubinodePaymentsStartBlock){
            CAmount subinodePayment = GetSubinodePayment(chainActive.Height() + 1, 0) + nShadeFees/10;
            FillBlockPayments(txNew, chainActive.Height() + 1, subinodePayment, pblock->txoutSubinode, pblock->voutSuperblock);

            for(int g = 2; g < 11; g++){
                CTxOut tempTx;
                mnpayments.FillBlockPayee(txNew, chainActive.Height() + g, nShadeFees/10, tempTx);
            }
        }
        //no subinode fees
        else{
            if (chainActive.Height() >= Params().GetConsensus().nSubinodePaymentsStartBlock) {
                CAmount subinodePayment = GetSubinodePayment(chainActive.Height() + 1, 0);
                FillBlockPayments(txNew, chainActive.Height() + 1, subinodePayment, pblock->txoutSubinode, pblock->voutSuperblock);
            }
        }
    }
    //Utilize new distribuition model
        ...
        int64_t returnFee = 0;
        bool payFees = false;
        //Check for shade fee distribution
        CBlock block;
        block.SetNull();
        if(!GetSubinodeFeePayment(returnFee, payFees, block))
            return error("%s: GetSubinodeFeePayment failed.", __func__);

        //Pay node winner block reward
        CAmount subinodePayment = GetSubinodePayment(chainActive.Height() + 1, 0);
        FillBlockPayments(txNew, chainActive.Height() + 1, subinodePayment, pblock->txoutSubinode, pblock->voutSuperblock);

        //add current block fee since we skip it in GetSubinodeFeePayment()
        returnFee += nShadeFees;

        //pay or dont pay the fees to all nodes
        if(payFees && returnFee != 0){
            vector<CSubinode> subinodeVector = mnodeman.GetFullSubinodeVector();

            int totalActiveNodes = 0;
            int startBlock = (chainActive.Height() + 1) - (Params().GetConsensus().nShadeFeeDistributionCycle - 1);
            int64_t ensureNodeActiveBefore = chainActive[startBlock]->GetBlockTime();

            for(auto node: subinodeVector){
...
bool CWallet::SignBlock(CBlockTemplate *pblocktemplate, int nHeight, int64_t nSearchTime)
{
    ...
    int64_t nShadeFees = 0;

    //check for zerocoin mints, start after coinbase
    if(chainActive.Height() + 1 > Params().GetConsensus().nSubinodePaymentsStartBlock){
        for(int i = 1; i < pblock->vtx.size(); i++){
            ...
            if(pblock->vtx[i]->IsZerocoinMint() && !pblock->vtx[i]->IsZerocoinSpend()){
                ...
                nShadeFees += mintAmount * 0.0025;

            }
        }
    }
    if(nShadeFees > nFees){
        LogPrintf("\nCWallet::SignBlock() ERROR: nShadeFees not able to payout, reverting to nFees, nShadeFees=%llf, nFees=%llf \n", nShadeFees, nFees);
        nShadeFees =  nFees;
        nFees = 0;
    }
    else
    ...
    //LogPrintf("\nShade Fees: nShadeFees=%llf, nFees=%llf \n", nShadeFees, nFees);
    ...
    if (CreateCoinStake(pblock->nBits, nSearchTime, nHeight, nFees, txCoinStake, key, pblocktemplate, nShadeFees, commitment, witnessroot))
    {
...
bool CWallet::FindUnloadedShadeTransactions(const CTransaction& tx)
{
