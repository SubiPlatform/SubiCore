...
UniValue getnewaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"address_type\"   (string, optional) The address type to use. Options are \"subinode\", \"p2sh-segwit(default)\", and \"bech32\". Default is set by -addresstype.\n"
            "\nResult:\n"
...
UniValue setaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The subi address to be associated with an account.\n"
...
static void SendMoney(CWallet * const pwallet, const CTxDestination &address, CAmount nValue, bool fSubtractFeeFromAmount, CWalletTx& wtxNew, const CCoinControl& coin_control/*, CScript shadeKey*/)
{
    CAmount curBalance = pwallet->GetBalance();
    ...
    /*
    //include public shade key in transaction if using a shade address
    if(!shadeKey.empty()){
        CRecipient recipientShade = {shadeKey, 0, fSubtractFeeFromAmount};
        vecSend.push_back(recipientShade);
    }
    */
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "                             The recipient will receive less subi than you enter in the amount field.\n"
    ...
    if (IsShadeAddress(request.params[0].get_str()))
    {
    ...
    */
    ...
    SendMoney(pwallet, dest, nAmount, fSubtractFeeFromAmount, wtx, coin_control/*, shadeKey*/);
...
UniValue leasestaking(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "\nLease an amount of subi to a certain address to stake.\n"
            + HelpRequiringPassphrase(pwallet) +
            "\nArguments:\n"
            "1. \"lease address\"                    (string, required) The subi address to lease stakes to.\n"
            "2. \"amount\"                              (numeric or string, required) The amount in " + CURRENCY_UNIT + " to send. eg 0.1\n"
            "3. \"fee percent\"                         (numeric, optional) The percentage to allow delegator to take. eg 11.9 (11.9%)\n"
            "4. \"lease percent reward address\"     (string, optional) The subi address to force lease fee stakes to.\n"
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4) {
        std::string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" \"address_type\" )\n"
            ...
            "}\n"
            "\nResult (DEPRECATED. To see this result in v0.16 instead, please start subid with -deprecatedrpc=addmultisigaddress).\n"
            "        clients should transition to the new output api before upgrading to v0.17.\n"
            "\"address\"                         (string) A subi address associated with the keys.\n"
...
UniValue addwitnessaddress(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("addwitnessaddress")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "addwitnessaddress is deprecated and will be fully removed in v0.17. "
            "To use addwitnessaddress in v0.16, restart subid with -deprecatedrpc=addwitnessaddress.\n"
...
void ListTransactions(CWallet* const pwallet, const CWalletTx& wtx, const std::string& strAccount, int nMinDepth, bool fLong, UniValue& ret, const isminefilter& filter)
{
    ...
    if ((!listSent.empty() || nFee != 0) && (fAllAccounts || strAccount == strSentAccount))
    {
        for (const COutputEntry& s : listSent)
        {
            ...
            entry.push_back(Pair("is_shaded", wtx.tx->IsZerocoinMint() || wtx.tx->IsZerocoinSpend()));
            ret.push_back(entry);
        }
    }
    ...
    if (listReceived.size() > 0 && wtx.GetDepthInMainChain() >= nMinDepth)
    {
        for (const COutputEntry& r : listReceived)
        {
            ...
            if (fAllAccounts || (account == strAccount))
            {
                ...
                if (fLong)
                    WalletTxToJSON(wtx, entry);
                entry.push_back(Pair("is_unshaded", wtx.tx->IsZerocoinSpend()));
                ret.push_back(entry);
            }
        }
    }
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    StartShutdown();
    return "wallet encrypted; Subi server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
UniValue lockunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending subi.\n"
...
UniValue getwalletinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"keypoololdest\": xxxxxx,         (numeric) the timestamp (seconds since Usubi epoch) of the oldest pre-generated key in the key pool\n"
    ...
    obj.push_back(Pair("shade_vault",       ValueFromAmount(pwallet->GetShadeBalance())));
    obj.push_back(Pair("shade_vault_unconfirmed",       ValueFromAmount(pwallet->GetShadeBalanceUnconfirmed())));
...
UniValue shadeamount(const JSONRPCRequest& request)
{
    CWallet *pwalletMain = GetWalletForJSONRPCRequest(request);

    if (request.fHelp || request.params.size() > 1)
        throw runtime_error("shadeamount <amount>(whole numbers only)\n" + HelpRequiringPassphrase(pwalletMain));

    int64_t nAmount = request.params[0].get_int64();

    bool strError = pwalletMain->ShadeModeMintTrigger(std::to_string(nAmount));

    if (!strError)
        throw JSONRPCError(RPC_WALLET_ERROR, "shadeamount");

    return "Sucessfully shaded " + std::to_string(nAmount) +  " SUBI";
}

UniValue unshadeamount(const JSONRPCRequest& request)
{
    CWallet *pwalletMain = GetWalletForJSONRPCRequest(request);

    if (request.fHelp || request.params.size() == 0 || request.params.size() > 2)
        throw runtime_error("unshadeamount <amount>(whole numbers only) <addresstosend>\n" + HelpRequiringPassphrase(pwalletMain));
    ...
    if (request.params.size() > 1){
        ...
        if(!IsShadeAddress(toKey))
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "rpcwallet unshadeamount(): Invalid toKey address");
    }
    ...
    std::string strError = pwalletMain->ShadeModeSpendTrigger(std::to_string(nAmount), toKey);
...
UniValue mintzerocoin(const JSONRPCRequest& request)
{
    ...
    return "Sucessfully shaded " + std::to_string(nAmount/COIN) +  " SUBI";
}
...
UniValue totalshaded(const JSONRPCRequest& request) {

    if (request.fHelp || request.params.size() > 1)
        throw runtime_error(
                "totalshaded\n"
                        "Returns total amount of shaded SUBI on the network\n");
...
UniValue enableTor(const JSONRPCRequest& request){
    ...
    if (request.fHelp || request.params.size() > 1)
        throw runtime_error(
                "enabletor <enable>(false/true)\n"
                        ...
                        "Please restart the SUBI daemon to update your changes");
    ...
    boost::filesystem::path pathTorSetting = GetDataDir()/"subitorsetting.dat";
    if(fStatus){
        if (WriteBinaryFileTor(pathTorSetting.string().c_str(), "enabled")) {
            result = ("Please restart the Subi Core wallet to route your connection to obfuscate your IP address. \nSyncing your wallet might be slower.");
        }else{
            result = ("Obfuscation cannot enable");
        }
    }else{
        if (WriteBinaryFileTor(pathTorSetting.string().c_str(), "disabled")) {
            result = ("Please restart the Subi Core wallet to disable IP obfuscation.");
        } else {
...
UniValue torStatus(const JSONRPCRequest& request){
    ...
    if (request.fHelp || request.params.size() > 1)
        throw runtime_error(
                "torstatus\n"
                        "Returns the status of tor obfuscation on your SUBI daemon");
...
UniValue manageaddressbook(const JSONRPCRequest &request)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_PARAMETER, _("Invalid SUBI address."));
...
UniValue getstakinginfo(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"moneysupply\": xxxxxxx,        (numeric) the total amount of SUBI in the network\n"
            "  \"reserve\": xxxxxxx,            (numeric) the total amount of SUBI in the network\n"
...
UniValue refillshadekeys(const JSONRPCRequest& request)
{

    CWallet *pwalletMain = GetWalletForJSONRPCRequest(request);

    if (request.fHelp || request.params.size() > 1)
        throw runtime_error("refillshadekeys <amount>(default=100)\n" + HelpRequiringPassphrase(pwalletMain));
    ...
    string fullKey;
    for(string key: shadeKey)
        fullKey+=key;

    UniValue results(UniValue::VARR);
    results.push_back("Sucessfully created shadekey amount: " + fullKey);
    return results;
}
    ...
    if (request.fHelp || request.params.size() > 1)
        throw runtime_error(
                "listunloadedpubcoins amount(default=all)\n"
                        "\nResults are an array of public shade keys:\n");
...
UniValue getpubcoinpack(const JSONRPCRequest& request) {
    ...
    if(keyAmount > listUnloadedPubcoin.size())
        throw JSONRPCError(RPC_WALLET_ERROR,
                           "Error: Not enough Commitment Keys, please run refillshadekeys");
...
UniValue payunloadedpubcoins(const JSONRPCRequest& request) {
    ...
    if(keyPack.IsValidPack())
        strError = pwalletMain->ShadeModeSpendTrigger(std::to_string(nAmount), "", keyPack.GetPubCoinPackScript());
    else
...
{ //  category              name                        actor (function)           argNames
    ...
    // Shade functions (experimental)
    { "Shade",             "listunspentshade",    &listunspentmintzerocoins,  {} },
    { "Shade",             "shadeamount",              &shadeamount,               {"amount"} },
    { "Shade",             "unshadeamount",            &unshadeamount,             {"amount"} },
    { "Shade",             "resetshade",          &resetmintzerocoin,         {} },
    { "Shade",             "setshadestatus",      &setmintzerocoinstatus,     {} },
    { "Shade",             "listshade",           &listmintzerocoins,         {"all"} },
    { "Shade",             "listpubcoins",             &listpubcoins,              {} },
    { "Shade",             "totalshade",             &totalshaded,              {} },

    { "Shade",             "refillshadekeys",          &refillshadekeys,           {"amount"} },
    { "Shade",             "listunloadedpubcoins",     &listunloadedpubcoins,      {"amount"} },
    { "Shade",             "payunloadedpubcoins",      &payunloadedpubcoins,       {"amount", "address"} },
    { "Shade",             "getpubcoinpack",           &getpubcoinpack,            {"amount"} },
    { "Shade",             "resetzerocoinamounts",     &resetzerocoinamounts,      {} },
    { "Shade",             "resetzerocoinunconfirmed", &resetzerocoinunconfirmed,  {} },
    { "Shade",             "listallserials",           &listallserials,            {"height"} },
    { "Shade",             "eraseusedzerocoindata",    &eraseusedzerocoindata,     {""} },
    { "Shade",             "encryptallzerocoins",      &encryptallzerocoins,       {""} },
    { "Shade",             "decryptallzerocoins",      &decryptallzerocoins,       {""} },
