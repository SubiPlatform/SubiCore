...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a SUBI address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and SUBI: URIs)"));
    ...
    subinodeAction = new QAction(platformStyle->SingleColorIcon(":/icons/about"), tr("&SubiNodes"), this);
    subinodeAction->setStatusTip(tr("Browse/enable subinodes"));
    subinodeAction->setToolTip(subinodeAction->statusTip());
    subinodeAction->setCheckable(true);
#ifdef Q_OS_MAC
    subinodeAction->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_5));
#else
    subinodeAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_5));
#endif
    tabGroup->addAction(subinodeAction);
    connect(subinodeAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(subinodeAction, SIGNAL(triggered()), this, SLOT(gotoSubinodePage()));
        // These showNormalIfMinimized are needed because Send Coins and Receive Coins
    // can be triggered from the tray menu, and need to show the GUI to be useful.
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        toolbar->addAction(subinodeAction);
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    subinodeAction->setEnabled(enabled);
...
void BitcoinGUI::gotoSubinodePage()
{
    QSettings settings;
    subinodeAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSubinodePage();
}
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to SUBI network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void BitcoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    ...
    */
    ...
    if(!subinodeSync.IsBlockchainSynced())
        {
...
void BitcoinGUI::setAdditionalDataSyncProgress(int count, double nSyncProgress)
{
    ...
    if(subinodeSync.IsBlockchainSynced())
    {
        ...
        if(subinodeSync.IsSynced(count)) {
            progressBarLabel->setVisible(false);
            progressBar->setVisible(false);
        ...
        strSyncStatus = QString(subinodeSync.GetSyncStatus().c_str());
...
{
    QString strTitle = tr("SUBI"); // default title
