...
#include <QSet>
#include <QTimer>
#include "../shade-address/stealth.h"
...
CAmount WalletModel::getShadeBalance() const
{
    return wallet->GetShadeBalance();
}

CAmount WalletModel::getShadeBalanceUnconfirmed() const
{
    return wallet->GetShadeBalanceUnconfirmed();
}
...
void WalletModel::checkBalanceChanged()
{
    ...
    CAmount newShadeBalance = getShadeBalance();
    CAmount newShadeBalanceUnconfirmed = getShadeBalanceUnconfirmed();
    ...
    if(cachedBalance != newBalance || cachedUnconfirmedBalance != newUnconfirmedBalance || cachedImmatureBalance != newImmatureBalance ||
        cachedWatchOnlyBalance != newWatchOnlyBalance || cachedWatchUnconfBalance != newWatchUnconfBalance ||
            cachedWatchImmatureBalance != newWatchImmatureBalance || cachedStaked != newStaked || cacheShadeBalance != newShadeBalance
            || cacheShadeBalanceUnconfirmed != newShadeBalanceUnconfirmed)
    {
        ...
        cacheShadeBalance = newShadeBalance;
        cacheShadeBalanceUnconfirmed = newShadeBalanceUnconfirmed;
        Q_EMIT balanceChanged(newBalance, newUnconfirmedBalance, newImmatureBalance,
                            newWatchOnlyBalance, newWatchUnconfBalance, newWatchImmatureBalance, newShadeBalance, newShadeBalanceUnconfirmed, newStaked, newWatchStakedBalance);
    }
}
        ...
        const std::string &purpose, ChangeType status)
{
    ...
    if(address.type() == typeid(CShadeAddress)){
        CShadeAddress sxAddr = boost::get<CShadeAddress>(address);
...
static void NotifyShadeProtocolChanged(WalletModel *walletmodel, CWallet *wallet,  const std::string &pubCoin, int denomination, const std::string &isUsed, ChangeType status)
{
    qDebug() << "NotifyShadeProtocolChanged:" + QString::fromStdString(pubCoin) + " " + QString::fromStdString(isUsed) + " status=" + QString::number(status);
    QMetaObject::invokeMethod(walletmodel, "updateAddressBook", Qt::QueuedConnection,
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyZerocoinChanged.connect(boost::bind(NotifyShadeProtocolChanged, this, _1, _2, _3, _4, _5));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyZerocoinChanged.disconnect(boost::bind(NotifyShadeProtocolChanged, this, _1, _2, _3, _4, _5));
...
UniValue CallRPC(std::string args, std::string wallet)
{
    ...
    for (size_t i = 0; i < args.size(); ++i)
    {
            ...
            && (c == ' ' || c == '\t'))
        {
            ...
            vArgs.push_back(subi::TrimQuotes(s));
            s.clear();
            continue;
        };
