...
const QString AddressTableModel::ShadeVault = "G";
...
public:
        ...
        cachedAddressTable.clear();
        {
            ...
            BOOST_FOREACH(const CZerocoinEntry& item, listPubcoin)
            {
                if(item.randomness != 0 && item.serialNumber != 0){
                    ...
                    cachedAddressTable.append(AddressTableEntry(AddressTableEntry::ShadeVault,
    ...
    void updateEntry(const QString &pubCoin, const QString &isUsed, int status)
    {
        ...
        int lowerIndex = (lower - cachedAddressTable.begin());
        bool inModel = (lower != upper);
        AddressTableEntry::Type newEntryType = AddressTableEntry::ShadeVault;
...
QVariant AddressTableModel::data(const QModelIndex &index, int role) const
{
    ...
    else if (role == TypeRole)
    {
        ...
        case AddressTableEntry::Receiving:
            return Receive;
        case AddressTableEntry::ShadeVault:
            return ShadeVault;
        default: break;
        }
    }
...
bool AddressTableModel::shadeSUBI(string &stringError, string denomAmount)
{
    WalletModel::UnlockContext ctx(walletModel->requestUnlock());
    if(!ctx.isValid())
    {
...
bool AddressTableModel::convertShade(string &stringError, string thirdPartyAddress, string denomAmount)
{
    WalletModel::UnlockContext ctx(walletModel->requestUnlock());
    if(!ctx.isValid())
    {
